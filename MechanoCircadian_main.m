...
inhibMag = 0;%0:.05:.6;%0:.02:.5;%0:.05:5;
...
% stiffnessVals = [1e7,300,100,30,10,3,1,.3,.1];%[30, 0.3, 30, 30, 30, 30, 30, 30, 30];%logspace(-1,4,20);
...
% contactArea = [2000, 500, 2000, 500, 1200, 2000, 1000, 1600, 900];
...
% popParam = popParam .* exp(sqrt(popVar)*randn(size(popParam)));
...
N = length(tInterp); 
freq = 0:Fs/N:Fs/2;
noiseLevel = [0,0,0];%[1e-4, 1e-4, 1e-4];
% measurementNoise = .1;
snr = 5;
...
for k = 1:length(stiffnessVals)
    ...
    for i = 1:numCells
        ...
        inhibVec(4) = 1*exp(randPop(i,112));%0.1; % fold overexpression of 5SA-YAP
        ...
        [periodCur,amplCur,tCur,yCur,rawOutput,oscDecayCur] =...
            conditionToOutputs(pCur,stiffnessVals(k),inhibVec,maxTime,randPop(i,1:108),noiseLevel);
        period(i) = periodCur(oscVarIdx);
        ampl(i) = amplCur(oscVarIdx);
        oscDecayRate(i) = oscDecayCur(oscVarIdx);
        % oscLogic(i) = oscDecayCur(oscVarIdx) > -8e-4;
...
%         plot(T/3600 - 48, Y(:,oscVarIdx))
        ...
        oscCur = awgn(oscCur, snr);%oscCur + measurementNoise*randn(size(oscCur));
        oscCur(oscCur<0) = 0;
        % plot(tInterp,oscCur)
        % drawnow
        if ~mod(i,10)
    ...
    refPeriod = NaN;%conditionToOutputs(pSol,stiffnessVals(k),inhibVec,maxTime);
...
%% plot single cell distributions
figure
% subplot(2,1,1)
...
%% anova on populations for stiffness
...
aov_period = anova(conditionsVec, periodVec)
...
numReplicates = 1;
rhoYAPTAZ = zeros(1,numReplicates);
pYAPTAZ = zeros(1,numReplicates);
rhoMRTF = zeros(1,numReplicates);
pMRTF = zeros(1,numReplicates);
for k = 1:numReplicates
    for i = inclIdx
        % curPowerFraction = randsample(popSeq{i}{7}, 100); %popSeq{i}{7};
        % curMRTF = randsample(popSeq{i}{5}, 100); %popSeq{i}{5};
        % curYAPTAZ = randsample(popSeq{i}{4}, 100); %popSeq{i}{4};
    ...
    [rhoYAPTAZ(k), pYAPTAZ(k)] = corr(log(meanYAPTAZ(inclIdx)'), log(meanPowerFraction(inclIdx)'))
    ...
    % xlim([.3 2.5])
    % ylim([.2 .9])
    % plot(YAPTAZOrigRange, meanPowerFracYAPTAZFit)
    % legend(conditionsStrings(inclIdx))
    xlabel('YAP/TAZ N/C')
    ylabel('Circadian power fraction')

    [rhoMRTF(k), pMRTF(k)] = corr(log(meanMRTF(inclIdx)'), log(meanPowerFraction(inclIdx)'))
    ...
    % xlim([.3 2.5])
    % ylim([.2 .9])
    % plot(MRTFOrigRange, meanPowerFracMRTFFit)
    % legend(conditionsStrings(inclIdx))
    xlabel('MRTF N/C')
    ylabel('Circadian power fraction')
    fprintf('%d of %d replicates\n', k, numReplicates)
end

%%
YAPTAZOrigRange = YAPTAZRange;
meanPowerFracYAPTAZFit = exp(polyval(polyYAPTAZ,log(YAPTAZRange)));
MRTFOrigRange = MRTFRange;
meanPowerFracMRTFFit = exp(polyval(polyMRTF,log(MRTFRange)));
