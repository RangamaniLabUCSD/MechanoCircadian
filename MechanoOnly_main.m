...
MRTFEqData = MRTFEqData(keepLogic);
MRTFEqStd = MRTFEqStd(keepLogic);
rng(100,'twister')
uqlab
paramNames = {'Kinsolo_MRTF','Kin2_MRTF',...
              'MRTFReleaseConst','Kcap','Kdim',...
              'JasMag','JasSens','StiffnessSens'};
MRTFParamInit = [1; 10; 100; 0.05; 0.05; 1; 0.2; 3.25];
% MRTFParamInit = [1; 10; 100; 3.25];
ModelOpts.Name = 'Forward model';
ModelOpts.mHandle = @forwardModel_MRTF;
myForwardModel = uq_createModel(ModelOpts);
numParam = 4;
PriorOpts.Name = 'Prior distribution on MRTF parameters';
for i = 1:numParam
    PriorOpts.Marginals(i).Name = paramNames{i};
    PriorOpts.Marginals(i).Type = 'Uniform';
    PriorOpts.Marginals(i).Parameters = [0.2 5];
end
% PriorOpts.Marginals(6).Parameters = [0.25 4];
myPriorDist = uq_createInput(PriorOpts);
myData.y = MRTFEqData;
myData.Name = 'MRTF nuclear to cytosolic ratio';
Solver.Type = 'MCMC';
Solver.MCMC.Sampler = 'AIES';
Solver.MCMC.NChains = 30;
Solver.MCMC.Steps = 2000;
Solver.MCMC.Visualize.Parameters = 4;
...
DiscrepancyOpts.Parameters = MRTFEqStd;
...
pSolMRTF = MRTFParamInit'.*modeVals;
...
for i = 1:4
    ...
    ksdensity(curSamples(:)*MRTFParamInit(i),...
        'Support','positive','BoundaryCorrection','reflection')
    xlabel(paramNames(paramNum))
end
...
for k = 1:size(popParamMRTF,1)
    pCurMRTF = popParamMRTF(k,:);
    for i = 1:length(stiffnessVals)
        inhibVec = [1,1,1,1, 0];
        stiffnessVec = [stiffnessVals(i),inf,0];
        SSVar = MechanoSS(stiffnessVec, inhibVec, pCurMRTF);
        MRTFEq(k,i) = SSVar(25)/SSVar(26);
    end
end

figure
errorbar(stiffnessTests(keepLogic), MRTFEqData(keepLogic), MRTFEqStd(keepLogic),...
    MRTFEqStd(keepLogic), 'LineStyle','none', 'LineWidth', 1, 'Marker', 's')
set(gca, 'XScale', 'log')
hold on
errorbar(stiffnessVals, mean(MRTFEq,1), std(MRTFEq,1), std(MRTFEq,1))
xlim([.1 200])
ylim([0 4])
